...
contract M8BountyFactory is usingOraclize{
    ...
    mapping(address => DeployedBounty) deployedBountiesMap;
   ...
   function isValidQuestion(uint _questionId, string _site) public payable{
    ...
    function createBounty(uint _questionId, string _site) public payable{
        require(_questionId != 0 && bytes(_site).length != 0);
        // require question doesn't already exist. store has of string _questionId
        ...
        DeployedBounty memory newBountyData = DeployedBounty({
            questionId: _questionId,
            site: _site
        });
        deployedBountiesMap[newBounty] = newBountyData;
...
contract M8Bounty is usingOraclize{
    ...
    function __callback(bytes32 queryId, string result) {
        // maybe amek function private and payable
        require(msg.sender == oraclize_cbAddress());
